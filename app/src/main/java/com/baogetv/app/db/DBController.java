package com.baogetv.app.db;

import android.content.Context;

import com.baogetv.app.db.domain.MyBusinessInfLocal;
import com.baogetv.app.db.domain.MyDownloadInfLocal;
import com.baogetv.app.db.domain.MyDownloadThreadInfoLocal;
...
import static com.baogetv.app.downloader.domain.DownloadInfo.STATUS_PAUSED;
...
public class DBController implements DownloadDBController {
    ...
    public List<DownloadInfo> findAllDownloading() {
        try {
            List<MyDownloadInfLocal> myDownloadInfLocals = myDownloadInfLocalDao.queryBuilder()
                    .where().ne("status", STATUS_COMPLETED).query();
    ...
    public List<DownloadInfo> findAllDownloaded() {
        try {
            List<MyDownloadInfLocal> myDownloadInfLocals = myDownloadInfLocalDao.queryBuilder()
                    .where().eq("status", STATUS_COMPLETED).query();
            return convertDownloadInfos(myDownloadInfLocals);
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void pauseAllDownloading() {
        try {
            ...
            myDownloadInfLocalIntegerUpdateBuilder.updateColumnValue("status", STATUS_PAUSED)
                    .where().ne("status", STATUS_COMPLETED);
    ...
    public void createOrUpdate(DownloadThreadInfo downloadThreadInfo) {
        try {
            myDownloadThreadInfoLocalDao.createOrUpdate(convertDownloadThreadInfo
                    (downloadThreadInfo));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private DownloadInfo convertDownloadInfo(MyDownloadInfLocal downloadInfLocal) {
        ...
        downloadInfo.setDownloadThreadInfos(
                convertDownloadThreadInfos1(downloadInfLocal.getDownloadThreadInfos()));
        return downloadInfo;
    }

    private List<DownloadThreadInfo> convertDownloadThreadInfos1(
