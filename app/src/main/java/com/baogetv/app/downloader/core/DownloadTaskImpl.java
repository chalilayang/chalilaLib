...
import com.baogetv.app.downloader.core.task.GetFileInfoTask;
import com.baogetv.app.downloader.core.thread.DownloadThread;
...
//  private volatile boolean isComputerDownload;
    ...
    public DownloadTaskImpl(ExecutorService executorService, DownloadResponse downloadResponse,
                            DownloadInfo downloadInfo, Config config, DownloadTaskListener
    ...
    public void start() {
        if (downloadInfo.getSize() <= 0) {
            ...
            for (DownloadThreadInfo downloadThreadInfo : downloadThreadInfos) {
                DownloadThread downloadThread = new DownloadThread(downloadThreadInfo,
                        downloadResponse,
                        config,
                        downloadInfo, this);
    ...
    public void onSuccess(long size, boolean isSupportRanges) {
        ...
        if (isSupportRanges) {
            ...
            for (int i = 0; i < threads; i++) {
                ...
                DownloadThreadInfo downloadThreadInfo = new DownloadThreadInfo(i, downloadInfo
                        .getId(),
                        downloadInfo
                                .getUri(), start,
                        end);
