...
public class ScaleCalculator {
     ...
     */
    private static float BASE_SCREEN_WIDTH = 720.0f;
    private static float BASE_SCREEN_HEIGHT = 1280.0f;
    ...
    public final void setBaseWidth(float baseWidth) {
        if (baseWidth > 0) {
            BASE_SCREEN_WIDTH = baseWidth;
        }
    }

    public final void setBaseHeight(float baseHeight) {
        if (baseHeight > 0) {
            BASE_SCREEN_HEIGHT = baseHeight;
        }
    }
    ...
    public final int scaleTextSize(float pixel) {
        ...
        if (ScaleCalculator.mCurScreenWidth / ScaleCalculator.BASE_SCREEN_WIDTH >= ScaleCalculator.mCurScreenHeight
                / ScaleCalculator.BASE_SCREEN_HEIGHT) {
    ...
    public final int scaleWidth(int pixel) {
        int scaleWidth = Math.round(this.getBaseSize(pixel)
                * ScaleCalculator.mCurScreenWidth
                / ScaleCalculator.BASE_SCREEN_WIDTH);
        return scaleWidth;
    }

    public final int scaleHeight(int pixel) {
        int scaleHeight = Math.round(this.getBaseSize(pixel)
                * ScaleCalculator.mCurScreenHeight
                / ScaleCalculator.BASE_SCREEN_HEIGHT);
     ...
     * @return
     */
    private final float getBaseSize(float pixel) {
        return pixel;
    }
    ...
    private final boolean isBaseHeight() {
        return ScaleCalculator.mCurScreenHeight == ScaleCalculator.BASE_SCREEN_HEIGHT;
    }

    private final boolean isBaseWidth() {
        return ScaleCalculator.mCurScreenWidth == ScaleCalculator.BASE_SCREEN_WIDTH;
    }
    ...
    private final void getScreenSize(Context context) {
        ...
        if (displayMetrics.widthPixels <= displayMetrics.heightPixels) {
            ...
            ScaleCalculator.mCurScreenHeight = this
